...
from tkinter.font import * # 기본외 폰트를 사용하기 위함
...
# 5. DB퍼리 함수를 정의
## 5-1. showDatas() - DB 학생장보 테이블에 데이터를 가져와 TreeView에 표시
def showDatas():
    '''
    데이터베이스 내 모든 학생정보를 가져와 표시하는 사용자함수\n
    매개변수 필요없음
    '''
    global dataView # 외부에 있는 변수를 전역변수로 사용

    ### DB연결. 커낵션 객체생성 -> 커서 -> 쿼리실행 -> 커서로 데이터 패치 -> 커서종료 -> 커넥션 종류
    conn = pymysql.connect(host=host, user=username, passwd=password, port=port, db=database)
    cursor = conn.cursor() # DB 쿼리 실행시 커서 실행
...
def getData(event):
    '''
    트리뷰 더블클릭으로 선택된 학생정보를 Entry 위젯에 채우는 사용자함수
    ...
    ## 엔트리 위젯 기존 내용 삭제
    ent1.config(state='normal')    # 데이터 들어갈 수 있게 함
    ent1.delete(0, END) # 학생번호 기존 데이터 삭제
    ent1.config(state='readonly')  # 다시 리드온리
...
def delData():
    ...
    if std_id == '':
        messagebox.showwarning('DELETE', '삭제 데이터 선택요망!')
        return
    ...
    try:
        ...
        print(e)
        conn.rollback() # 트랜잭션 롤백
        messagebox.showerror('UPDATE', '학생삭제 실패!')
    finally:
        cursor.close() # 커서 종료
        conn.close()   # 커넥션 종료
...
myFont = Font(family='NanumGothic', size=10) # 화면 위젯에 지정할 폰트 생성
...
# ent1.config(state='readonly') # 값을 입력 못하게 방지
...
tk.Button(root,text='추가', font=myFont, height=2, width=12, command=addData).place(x=30, y=130) # 추가 버튼
## 8-1. 수정버튼에 modData() 함수를 연결
tk.Button(root,text='수정', font=myFont, height=2, width=12, command=modData).place(x=140, y=130) # 수정 버튼
## 9-1. 삭제버튼에 delData() 함수를 연결
tk.Button(root,text='삭제', font=myFont, height=2, width=12, command=delData).place(x=250, y=130) # 삭제 버튼
## 4-4. 트리뷰(행과 열로 만들어진 데이터 표현할 때 좋은 위젯중 하나)
cols = ('학생번호', '학생명', '핸드폰', '입학년도')
dataView = ttk.Treeview(root, columns=cols, show='headings', height=14)

## 4-5. 트리뷰 설정
for col in cols:
    dataView.heading(col, text=col)                 # 각 열 제목을 cols에 변수하나씩 지정
    dataView.grid(row=1, column=0, columnspan=2)    # 트리뷰 위젯을 그리드 레이아웃에 배치
    dataView.place(x=10, y=180)                     # 트리뷰 위젯배치


# 5. showData() 호출
showDatas()

# 6. 트리뷰항목 더블클릭하면 이벤트 발생(getData()함수 호출)
