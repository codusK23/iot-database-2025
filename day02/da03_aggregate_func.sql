...
-- GROUP BY 를 사용하면 반드시 집계함수 및 GROUP BY에 포함된 컬럼이 SELECT 안에 포함되어야 함.
...
-- 3-20 가격이 8000원 이상인 도서를 구매한 고객에 대하며 고객별 주문도서의 총 수량을 구하시오
-- 단, 2권이상 구매한 고객에 대해서만 한정
-- COUNT()등 집계함수는 WHERE 절에 넣을 수 없음
SELECT custid
	 , COUNT(*) AS 총수량
  FROM Orders 
...
SELECT name, b.bookname
 ...
 WHERE b.bookid = o.bookid
   AND o.custid = c.custid	-- JOIN을 위한 조건
...
-- LEFT OUTER JOIN 또는 RIGHT OUTER JOIN - LEFT, RIGHT는 기준이 되는 테이블 위치xa
SELECT *
  FROM Customer AS c
