...
-- 파이썬, C#등 프로그래밍언어로 가져갈땐, 컬럼이름 컬럼갯수를 모두 파악해야 하기 때문에 아래와 같이 사용
...
SELECT *
 ...
 -- 3-5 : 가격이 10000원이상 20000원이하인 도서를 검색하시오.
SELECT *
  FROM Book
...
-- 패턴
-- % 갯수와 상관없이 글자가 포함되는 것
-- [0-5] 1개의 문자가 일치, 한글자 0에서 5의 숫자를 포함한다. # MySQL 에서는 사용불가
-- [^] 1개의 문자가 일치하지 않는 것 # MySQL 에서는 사용불가
-- _ 특정위치의 1개의 문자가 일치할 때
-- 		_구% : 첫번째글자는 뭐든지 상관없고, 두번째 글자가 구로되고 뒤에 글이 더 있다
